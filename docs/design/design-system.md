...
- 地圖圖片不在地址輸入過程中即時刷新；使用者輸入完地址後點「取得清晰地圖圖片」，系統才產生固定尺寸地圖預覽。
...
- Title 下方提供角色可見的建案篩選區：出租狀態、租金區間、建案類型、國家、城市；篩選只影響卡片清單顯示，不修改資料。
- 建案預覽圖（地圖或外觀照）可點擊開啟高解析放大檢視，Esc 或點擊背景可關閉。
...
- 每張建案外觀照片提供兩種來源按鈕：
  ...
  - 「使用預設 / Google Place」：系統先用地址查 Place 照片；若該地址結果沒有 photos，依序改查附近地標、街景、大樓名／建築關鍵字與建案名稱；全部沒有照片時才將該張預設建案圖片存為空值。取得照片後預覽，儲存時裁切成適當大小；若瀏覽器 CORS 無法下載裁切，先保留 Google 照片 URL 顯示，後續可由後端代理補強。
...
- 非管理員只能修改自己的顯示名稱、電話、寄件地址與提醒設定；角色、建案、房號維持唯讀並由管理員調整。
